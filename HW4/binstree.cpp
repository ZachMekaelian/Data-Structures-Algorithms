...
bool BST::PREorderSearch(Vertex* V, el_t K)
{
    if (V != NULL)
    {
        if (K == V->Elem) { return true; } // found the element in V
            ...
            V = V->Left;
            return PREorderSearch(V, K); // traverse left sub-tree of V recursively
        }
        else {
            V = V->Right;
            return PREorderSearch(V, K);// traverse right sub-tree of V recursively
        }
    }
    else 
        return false;
...
// case 2:  One Substree Root ----------------
       else if (Root->Right == NULL && Root->Left != NULL)// if what you want to delete is the root
       {
  ...
  else // V is a right child of the Parent
  {
     ...
     {   
      ...
      if (V == P->Left)            //** You need if then else to determine Parent's left or right
          cout << "V is the left child of P" << endl;
      else
          cout << "V is the right child of P" << endl;
	       //should point to C; 
      C->Elem = C->Up->Elem;//** Make C point UP to the parent;
     ...
     {    Vertex* C = V->Right;  // C is the right child
      ...
      if (V == P->Left)
          ...
          cout << "V is the right child of P" << endl;//** You need if then else to determine Parent's left or right
	         //    should point to C; 
            C->Up->Elem = C->Elem;  //** Make C point UP to the parent;   
          cout << C->Elem << " points up to " << C->Up->Elem << endl;
          delete V; // Be sure to delete V
          //** EC call from P to adjust height and BF 
